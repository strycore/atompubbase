...
class Test(unittest.TestCase):
    def _parse(self, output):
        parsed = [tuple(l.split(" ", 1)[0].split(":"))
    ...
    def testNonWellFormed(self):
        ...
        """
        output = Popen([
            "python", "./validator/appclienttest.py", "--quiet",
            "--playback=./validator/rawtestdata/invalid-service/"
        ], stdout=PIPE).communicate()[0]
        parsed, msg_count = self._parse(output)
        # FIXME : not sure the distant service responds
    ...
    def testNoLocation(self):
        ...
        On Entry creation neither a Location or Content-Location: header are
        returned.
        """
        output = Popen([
            "python", "./validator/appclienttest.py",
            "--quiet", "--playback=./validator/rawtestdata/nolocation/",
